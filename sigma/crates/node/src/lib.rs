...
impl From<ChangedContent> for websegment_core::ChangedContent {
    fn from(changed_content: ChangedContent) -> Self {
        websegment_core::ChangedContent {
...
#[napi]
pub fn clear_cache() {
    websegment_core::clear_cache();
}

#[napi]
pub fn scan_dir(args: ScanOptions) -> ScanResult {
    let result = websegment_core::scan_dir(websegment_core::ScanOptions {
        base: args.base,
...
pub fn scan_files(input: Vec<ChangedContent>, strategy: u8) -> Vec<String> {
    websegment_core::scan_files(input.into_iter().map(Into::into).collect(), strategy)
}
