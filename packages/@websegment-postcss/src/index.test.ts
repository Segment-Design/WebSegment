...
// and packages/websegment must be a sub-folder for
// @import 'websegment' to work.
const INPUT_CSS_PATH = `${__dirname}/fixtures/example-project/input.css`

const css = String.raw

beforeEach(async () => {
  const { clearCache } = await import('@websegment/sigma')
  clearCache()
})

test("`@import 'websegment'` is replaced with the generated CSS", async () => {
  let processor = postcss([
    websegment({ base: `${__dirname}/fixtures/example-project`, optimize: { minify: false } }),
  ])

  let result = await processor.process(`@import 'websegment'`, { from: INPUT_CSS_PATH })
...
test('output is optimized by Lightning CSS', async () => {
  ...
  // `@apply` is used because Lightning is skipped if neither `@websegment` nor
  // `@apply` is used.
...
test('@apply can be used without emitting the theme in the CSS file', async () => {
  ...
  let result = await processor.process(
    css`
      @import 'websegment/theme.css' reference;
      .foo {
        @apply text-red-500;
      }
    `,
...
describe('processing without specifying a base path', () => {
  ...
  test('the current working directory is used by default', async () => {
    let processor = postcss([websegment({ optimize: { minify: false } })])
