...
export function compile(css: string): {
  ...
  if (process.env.NODE_ENV !== 'test') {
    ast.unshift(comment(`! websegment v${version} | MIT License | https://websegment.com `))
  }
  ...
  walk(ast, (node, { replaceWith }) => {
    ...
    //
    // We support `@import "websegment/theme" reference` as a way to import an external theme file
  ...
  let websegmentUtilitiesNode: Rule | null = null

  // Find `@websegment utilities` and replace it with the actual generated utility
  // class CSS.
  walk(ast, (node) => {
    if (node.kind === 'rule' && node.selector === '@websegment utilities') {
      websegmentUtilitiesNode = node

      // Stop walking after finding `@websegment utilities` to avoid walking all
      // of the generated CSS. This means `@websegment utilities` can only appear
  ...
  return {
    build(newRawCandidates: string[]) {
      ...
      if (websegmentUtilitiesNode) {
        ...
        websegmentUtilitiesNode.nodes = newNodes
